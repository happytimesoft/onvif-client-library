...
typedef struct
{
    ...
    uint32  CloudFirmwareUpgrade: 1;                    // Indicates support for firmware upgrade through the cloud
...
typedef struct
{
    ...
    uint32  KeyPairGenerationFlag : 1;                  // Indicates whether the field KeyPairGeneration is valid
    ...
    char    KeyPairGeneration[100];                     // optional, List of supported key algorithm like 'RSA' and 'ECC'.
...
typedef struct
{
    ...
    uint32  Mirror      : 1;                            // When enabled, the video will be flipped horizontally. If applied alongside rotation,
                                                        //  the mirror effect shall be executed after the rotation. Additionally,
                                                        //  when Mirror is enabled and Reverse=Auto is set in PTControlDirection or if the device doesn't support 
                                                        //  Reverse in PTControlDirection, the device shall automatically adjust the pan direction.
    uint32  Reserved    : 30;
...
typedef struct
{
    uint32  ViewModeFlag        : 1;                    // Indicates whether the field ViewMode is valid
    ...
    char    ViewMode[32];                               // optional, Readonly parameter signalling Source configuration's view mode
                                                        //  tt:Fisheye - Undewarped viewmode from device supporting fisheye lens
                                                        //  tt:360Panorama - 360 degree panoramic view
                                                        //  tt:180Panorama - 180 degree panoramic view
                                                        //  tt:Quad - View mode combining four streams in single Quad, eg., applicable for devices supporting four heads
                                                        //  tt:Original - Unaltered view from the sensor
                                                        //  tt:LeftHalf - Viewmode combining the left side sensors, applicable for devices supporting multiple sensors
                                                        //  tt:RightHalf - Viewmode combining the right side sensors, applicable for devices supporting multiple sensors
                                                        //  tt:Dewarp - Dewarped view mode for device supporting fisheye lens
...
typedef struct 
{
    ...
    uint32  Mirror          : 1;                        // Signals if video source mirroring is supported
    uint32  Reserved        : 27;
...
typedef struct
{
    uint32  KeyRotationDurationFlag : 1;                // Indicates whether the field KeyRotationDuration is valid
    uint32  Reserved : 31;
    
    uint32  sizeCertificateID;                          // sequence of elements <CertificateID>
    char    CertificateID[4][64];                       // List of certificates used to encrypt the symmetric key for the PSSH box
    uint32  KeyRotationDuration;                        // Frequency at which the device shall generate a new key to encrypt a new segment. 
                                                        //  If not specified, key rotation is disabled. 
                                                        //  KeyRotationDuration must be a positive duration value
} onvif_AsymmetricEncryption;

typedef struct 
{
    uint32  KeyFlag     : 1;                            // Indicates whether the field Key is valid
    uint32  AsymmetricEncryptionFlag : 1;               // Indicates whether the field AsymmetricEncryption is valid
    uint32  Reserved    : 30;
    ...
    onvif_AsymmetricEncryption AsymmetricEncryption;    // optional
...
{
    uint32  AliasFlag : 1;                              // Indicates whether the field Alias is valid
    uint32  HasPrivateKey : 1;                          // The keystore has a matching private key
    uint32  Reserved  : 30;
